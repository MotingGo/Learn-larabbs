...
return array(
     ...
     * 3. 配置信息 —— 必须使用前缀 `settings.`，对应 `settings_config_path` 目录下的文件，如：默认设置下, `setting.site` 访问的是 `administrator/settings/site.php` 文件
     * 4. 页面文件 —— 必须使用前缀 `page.`，如：`page.pages.analytics` 对应 `administrator/pages/analytics.php` 或者收 `administrator/pages/analytics.blade.php`, 两种后缀名皆可
     *
     * 示例：
     * [
     *      'users',
     *      'E-commerce' => ['collections', 'products', 'product_image', 'orders'],
     *      'Settings' => ['settings.site', 'settings.ecommerce', 'setting.social'],
     ...
     * 使用布尔值来设定是否使用后台主页面
     *
     * 如值为 `true`，将使用 `dashboard_view` 定义的试图文件渲染页面；
     * 如值为 `false`，将使用 `home_page` 定义的菜单条目来作为后台主页。
     */
    'use_dashboard' => false,

    // 设置后台主页试图文件，由 `dashboard_view` 选项决定
    'dashboard_view' => '',
    ...
    // 允许再登陆成功后使用 Session::get('redirect') 将用户重定向到原本想要访问的后台页面
